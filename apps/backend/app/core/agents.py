...
async def highlighter_node(state: CogmateState) -> CogmateState:
    ...
    try:
        ...
        # Use only tags for state['importance_tags'] but we could store scores if needed
        tags = [c['tag'] for c in concepts if c.get('score', 0) >= 3]
    except Exception:
        tags = ["Neural Networks"] if any("Neural" in t for t in state["transcript_buffer"]) else []
