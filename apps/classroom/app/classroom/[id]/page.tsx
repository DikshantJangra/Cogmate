...
export default function ClassroomDetailPage() {
  ...
  const loadClassroom = useCallback(async () => {
    try {
      ...
      // Final check: if stage is still null, it doesn't exist anywhere
      if (!useStageStore.getState().stage) {
        log.error('Classroom not found after trying storage and API:', classroomId);
        setError('Classroom not found. It may have been deleted or the link is invalid.');
        setLoading(false);
        return;
      }
        ...
        <div className="h-screen flex flex-col overflow-hidden bg-white">
          {loading ? (
            <div className="flex-1 flex flex-col items-center justify-center space-y-4">
              ...
              <p className="text-[12px] text-slate-400 font-medium uppercase tracking-[0.2em]">Preparing Classroom...</p>
            </div>
          ) : error ? (
