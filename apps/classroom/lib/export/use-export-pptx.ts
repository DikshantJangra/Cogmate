  ...
  const parse = (obj: AST[], baseStyleObj: Record<string, string> = {}) => {
    for (const item of obj) {
      ...
      if ('tagName' in item && item.tagName === 'br') {
        ...
        if (styleObj['text-align']) options.align = styleObj['text-align'] as any;
...
function getShadowOption(shadow: PPTElementShadow, ratioPx2Pt: number): any {
...
function getOutlineOption(outline: PPTElementOutline, ratioPx2Pt: number): any {
...
function getLinkOption(link: PPTElementLink, slides: Slide[]): any | null {
    ...
    for (const el of slide.elements) {
      ...
      // ── SHAPE ──
        ...
        if (el.special) {
          ...
          pptxSlide.addShape('custGeom' as any, shapeOptions);
        }
      ...
      // ── LINE ──
        ...
        pptxSlide.addShape('custGeom' as any, lineOptions);
      }
      ...
      // ── CHART ──
        ...
        pptxSlide.addChart(type, chartData as any, chartOptions);
      }
