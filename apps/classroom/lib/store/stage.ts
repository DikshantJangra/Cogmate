...
interface StageState {
  ...
  exportResourcePack: () => Promise<void>;
...
const useStageStoreBase = create<StageState>()((set, get) => ({
  ...
  exportPPTX: async () => {
    ...
    log.info('Mock: Exporting PPTX...');
    await new Promise(resolve => setTimeout(resolve, 2000));
    set({ isExporting: false });
    log.info('Mock: PPTX Export Complete');
  },

  exportResourcePack: async () => {
    set({ isExportingZip: true });
    log.info('Mock: Exporting Resource Pack...');
    await new Promise(resolve => setTimeout(resolve, 2000));
    set({ isExportingZip: false });
    log.info('Mock: Resource Pack Export Complete');
  },
