...
import { PedagogicalPulse } from './stage/pedagogical-pulse';
import { SessionSummary } from './stage/session-summary';
...
export function Stage({
  ...
  return (
    <div
      ref={stageRef}
      className={cn(
        'flex-1 flex overflow-hidden bg-slate-50/40 text-slate-900 font-sans',
    ...
    >
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0 relative">
        ...
        {/* Canvas Area and Pedagogical Pulse */}
        <div
          className="flex flex-1 min-h-0 overflow-hidden relative isolate px-6 pb-6"
          style={{
            height: sceneViewerHeight,
          }}
          suppressHydrationWarning
        >
          {isPendingScene && isCourseComplete ? (
            <div className="flex-1 overflow-hidden bg-white rounded-[32px] border border-slate-100 shadow-2xl shadow-slate-200/50 mt-2">
              <SessionSummary />
            </div>
          ) : (
            <div className="flex flex-1 gap-6 mt-2">
              <div className="flex-1 relative overflow-hidden bg-white rounded-[32px] border border-slate-100 shadow-2xl shadow-slate-200/50">
                <CanvasArea
              ...
              {/* Pedagogical Pulse Sidebar - Now as a floating card */}
              {!isPresenting && mode === 'playback' && (
                <div className="shrink-0 w-80 bg-white rounded-[32px] border border-slate-100 shadow-2xl shadow-slate-200/50 overflow-hidden">
                  <PedagogicalPulse />
                </div>
              )}
            </div>
        ...
        {/* Roundtable Area - Now as a floating bar with margin */}
        {mode === 'playback' && (
          <div
            className={cn(
              'transition-all duration-500 px-6 pb-6 shrink-0',
              isPresenting && 'absolute inset-x-0 bottom-0 z-20 px-0 pb-0',
            )}
          >
